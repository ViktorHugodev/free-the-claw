#!/usr/bin/env node
// ComfyUI Cloud — LTX-2 Music Video generator
// Generates a 5-second video of blinking stars and galaxies with sung lyrics
// Usage: node make-music-video.js "your lyrics here" [--output video.mp4] [--seed 42]
...
function buildWorkflow(
  lyrics,
  { seed = Math.floor(Math.random() * 2 ** 32), fps = 24 } = {},
) {
  // 5 seconds at 24fps = 120 frames; LTX-2 needs frames+1 = 121
  const frames = 181;
  const visualPrompt =
    `A breathtaking cosmic scene predominantly blues and reds of deep space with brilliant blinking stars, swirling galaxies, and colorful nebulae. ` +
    `Stars twinkle and pulse with light against the vast blue and red universe. Camera very slowly drifts through the starfield. ` +
    `Ethereal and dreamlike atmosphere, cinematic lighting, high detail. ` +
    `A soft slow female voice is singing the following lyrics: "${lyrics}"`;

  return {
    75: {
      inputs: {
        filename_prefix: "video/music-video",
        format: "mp4",
        codec: "auto",
        video: ["92:97", 0],
      },
    ...
    "92:4": {
      inputs: {
        text: "blurry, low quality, still frame, frames, watermark, overlay, titles, has blurbox, has subtitles, static image, no motion",
        clip: ["92:60", 0],
      },
      class_type: "CLIPTextEncode",
    },
    "92:89": {
      inputs: { width: 640, height: 360, batch_size: 1, color: 0 },
    ...
    "92:3": {
      inputs: { text: visualPrompt, clip: ["92:60", 0] },
      class_type: "CLIPTextEncode",
    },
    "92:97": {
...
async function pollStatus(promptId, intervalMs = 5000, maxWaitMs = 600000) {
  ...
  while (Date.now() - start < maxWaitMs) {
    ...
    process.stderr.write(
      `[${data.status}] ${((Date.now() - start) / 1000).toFixed(0)}s\n`,
    );
...
async function main() {
  ...
  if (!args.lyrics) {
    console.error(
      'Usage: node make-music-video.js "lyrics to sing" [--output file.mp4] [--seed N]',
    );
    process.exit(1);
  }

  console.error(`Lyrics:  ${args.lyrics}`);
  console.error(`Seed:    ${args.seed}`);
  console.error(`Output:  ${args.output}`);

  const workflow = buildWorkflow(args.lyrics, { seed: args.seed });
  ...
  console.error(
    `Saved ${(bytes / 1024 / 1024).toFixed(1)} MB to ${args.output}`,
  );
